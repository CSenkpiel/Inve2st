...
iterables_on = False
#iterables_on = True                                                          # used for plotting options so if iterables specicified use True else False 

#demand_scenarios = ['Row_house_Family_4P','single_family_house_2_persons']    # available demand scenarios    
#demand_scenarios = ['Row_house_Family_4P']
demand_scenarios = ['single_family_house_2_persons']


#regions =['DE731','DEF01','DE131']                                            # available regions for PV generation #DE731 (Kassel), DEF01 (Flensburg), DE131 (Freiburg)
regions =['DE131']

#PV_orientations= ['S1_pv_eastwest','S2_pv_renewable_ninja_south']             # available PV orientations
PV_orientations= ['S2_pv_renewable_ninja_south']

# Dimensioning of PV-battery system 
#ratio PV-size to electricity demand
#ratio_PV_demands = [0.8, 1, 1.2, 1.4]                                         # suggestions of ratios Figgener.2018
ratio_PV_demands = [1.2]

# PV size to Battery capacity
#ratio_PV_Batterys = [0.8,1,1.5,2]                                             # suggestions of ratios (between 0.8 and 5 / around 1/ 1.5)
ratio_PV_Batterys = [0.8]                                                      

ratio_PV_energy_power = 4                                                      

roundtrip_efficiency_bat = 0.85                                                # Figgener.2018 (0.75 - 0.95)
...
#start_years = [2020,2021,2022,2023,2024,2025,2026,2027,2028,2029,2030,2040,2050]
start_years = [2050]

#____economic calculations_____________________________________________________
technical_lifetime_PV = 20
technical_lifetime_battery = 10
...
#['S1_PV_high_3_to10kW', 'S1_PV_high_less_3kW', 'S1_PV_low_3_to10kW', 'S1_PV_low_less_3kW']
scenario_CAPEX_PVs = ['S1_PV_low_3_to10kW']

#scenario_CAPEX_bats = ['S2_battery_high_path_6to12kWh', 'S2_battery_high_path_upto6kWh', 'S2_battery_low_path_6to12kWh', 'S2_battery_low_path_upto6kWh']
scenario_CAPEX_bats = ['S2_battery_low_path_6to12kWh']

#scenario_FITs = ['S_0_percent_degression','S_05_percent_monthly_degression','S_1_percent_monthly_degression','S_2_percent_monthly_degression','S_2_8_percent_monthly_degression']
scenario_FITs = ['S_0_percent_degression']

#scenario_eex_prices = ['EEX_low','EEX_high']
scenario_eex_prices = ['EEX_high'] 

#scenario_consumption_prices = ['S3_high_price_hh', 'S1_low_price_hh','S2_medium_price_hh']
scenario_consumption_prices = ['S3_high_price_hh', 'S1_low_price_hh']
#scenario_consumption_prices = ['S3_high_price_hh']


installation_costs = 1500 # Costs are between 99-2300 € (today)
...
def calculate_PV_battery_use(demand_scenario,region, PV_orientation):
    ...
    charge_cap = battery_cap / ratio_PV_energy_power
    discharge_cap = battery_cap / ratio_PV_energy_power
    ...
    for i in range(len(df_PV_use)):
        if df_PV_use.loc[i,'residual_load_w_stor'] >= 0:                                                                # not Feed in with positive Residual load (Unterschuss)
...
def calculate_cashflow(technical_lifetime_PV, technical_lifetime_battery, start_year, investment_option, scenario_CAPEX_PV,scenario_CAPEX_bat, scenario_FIT,scenario_eex_price, scenario_consumption_price, WACC, load_sum,gen_PV_w_stor_sum,Degree_autarky_wo_stor, Degree_autarky_w_stor):
    """
    ...
    for i in range(1, temp_tec_lifetime+1):
        ...
        df_energy_balance.loc[i,'self_consumption_PV'] = df_energy_balance.loc[i,'net_generation'] * Degree_autarky_wo_stor
        df_energy_balance.loc[i,'self_consumption_batt'] = df_energy_balance.loc[i,'net_generation'] * (Degree_autarky_w_stor - Degree_autarky_wo_stor)
        df_energy_balance.loc[i,'grid'] = df_energy_balance.loc[i,'net_generation'] * (1-Degree_autarky_wo_stor - Degree_autarky_w_stor)
...
if iterables_on == False: 
    ...
    #fig.show()
    fig.write_html('cumulated_cash_flow.html', auto_open=True)
